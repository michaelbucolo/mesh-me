...
function reason(query: string, context?: ChatRequest["context"]): ReasonResult {
  ...
  // posting". Neither was true as built. The platform names below must stay
  // inside lib/platforms.ts's allow-list and match what the capability table
  // actually grants — Meshi is the surface people ASK what the product does,
  // so a confident wrong answer here is worse than the same wrong answer in a
  // headline — it was requested. scripts/unified-claim-check.ts holds this.
  if (q.includes("feature") || q.includes("what can")) return { content: "mesh.me has three main areas: The Mesh (visualize & manage your digital footprint), Flow (content from the platforms that allow importing — X, YouTube, Twitch, TikTok, Reddit and Discord today), and MeChat for your mesh.me conversations. Plus: per-node privacy controls, achievement badges, Meshi (that's me!), communities, and MeshPro extras. The One Account page shows exactly what each platform does and does not let mesh.me do. What would you like to know more about?", mood: "excited" };
  ...
  if (q.includes("platform") || q.includes("connect") || q.includes("instagram") || q.includes("youtube") || q.includes("tiktok")) return { content: "Connect the platforms people actually use — Instagram, TikTok, YouTube, X, Threads, Facebook, Snapchat, Discord, Twitch, Reddit, LinkedIn, and Pinterest. Mesh.me imports through official APIs where allowed and keeps restricted platforms read-only until provider approval and scopes are available.", mood: "excited" };
