          ...
          {followers.map((f) => {
            ...
            return (
              <div key={f.id} className="glass-card rounded-xl p-4 flex items-center gap-3 hover-lift">
                <div className="relative">
                  {f.avatarUrl ? (
                    <NextImage src={f.avatarUrl} alt={f.displayName || f.username || ""} width={40} height={40} className="h-10 w-10 rounded-full object-cover" />
                  ) : (
      ...
      <div className="flex items-start gap-3">
        ...
        {post.thumbnailUrl && !compact && (
          <div className="h-16 w-24 rounded-lg overflow-hidden flex-shrink-0 bg-[var(--bg-tertiary)]">
            <NextImage src={post.thumbnailUrl} alt="" width={96} height={64} className="h-full w-full object-cover" />
          </div>
        )}
