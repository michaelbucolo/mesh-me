      ...
      <div className="px-4 py-6">
        ...
        {activeTab === "about" && (
          <div className="space-y-6">
            {profile.bio && (
              <div>
                <h3 className="text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-wide mb-2">Bio</h3>
            ...
            <div className="grid gap-3">
              ...
              {profile.website && (
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <a href={profile.website} target="_blank" rel="noopener noreferrer" className="transition-colors" style={{ color: "var(--accent)" }}>
            ...
            {profile.interests.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-wide mb-2">Interests</h3>
            ...
            {connectedAccounts.length > 0 && (
              <div>
                <h3 className="text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-wide mb-2">Connected Platforms</h3>
                <div className="grid gap-2">
                  {connectedAccounts.map((account) => (
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-medium text-[var(--text-primary)] capitalize">{account.platform}</p>
                        {account.platformUsername && (
                          <p className="text-xs text-[var(--text-muted)]">@{account.platformUsername}</p>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          communities.length > 0 ? (
            <div className="space-y-2">
              {communities.map((community) => (
                ...
                >
                  <div className="h-10 w-10 rounded-xl flex items-center justify-center text-white font-bold text-sm" style={{ background: "var(--brand-gradient)" }}>
                    {community.name[0]}
                  </div>
                  <div className="flex-1">
                    <h4 className="text-sm font-semibold text-[var(--text-primary)]">{community.name}</h4>
                    <p className="text-xs text-[var(--text-muted)]">{community.memberCount} members</p>
                  </div>
