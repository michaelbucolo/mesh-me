    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p>Mesh.me also offers one-time purchases: gifting MeshPro time to another account, individual Meshi wardrobe pieces ($1.99 each, for your own account or another person&apos;s), and one hundred numbered Charter seats. A gift grants the recipient prepaid MeshPro time that no subscription change can revoke. A wardrobe piece is a cosmetic owned outright — it stays wearable with or without MeshPro, grants no other capability, and if its purchase is fully refunded the piece is revoked. A Charter seat is a status purchase only — it grants a permanent seat number and its cosmetic marks, and deliberately no features; if a Charter purchase is fully refunded, its number is retired and never resold.</p>
          <p>Mesh.me also offers Patron, a recurring monthly contribution ($2, $5, or $10) that deliberately buys no features — only its cosmetic marks, a profile chip and a Meshi pin, which record that the account has supported Mesh.me. Cancelling stops future charges and removes nothing; only a full refund erases the record.</p>
          <p>We may change subscription pricing or features on a going-forward basis; changes will not affect the period you have already paid for, and we will provide notice of price changes before your next renewal. Mesh.me does not display advertisements or sell user data — MeshPro (including gifts), one-time Meshi wardrobe pieces, the one hundred Charter seats, and recurring Patron contributions are the platform&apos;s only revenue sources, and this is a core product principle rather than a temporary policy.</p>
        </>
      ),
    },
    {
