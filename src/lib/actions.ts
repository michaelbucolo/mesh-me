...
import { rateLimit, sanitizeForDisplay, validatePasswordStrength, validateUrl } from "./security";
import { canViewNsfw, getNsfwPolicyForRegion, isAdultVerificationActive, normalizeUsState } from "./content-safety";
...
import { createPostAsUser } from "./post-core";
...
export async function createPost(formData: FormData) {
  const user = await getCurrentUser();
  if (!user) return { error: "Not authenticated" };
  // The whole body lives in post-core.ts (createPostAsUser) so the scheduled
  // fire path obeys the SAME laws a live post does — one definition, not a
  // second weaker copy. This wrapper only establishes who is asking.
