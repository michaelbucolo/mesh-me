    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="presence-meshi-chat fixed bottom-[calc(5.75rem+env(safe-area-inset-bottom))] right-3 z-50 flex h-[min(520px,calc(100dvh-7rem-env(safe-area-inset-bottom)))] min-h-0 max-h-[calc(100dvh-7rem-env(safe-area-inset-bottom))] w-[calc(100vw-1.5rem)] max-w-[360px] flex-col overflow-hidden rounded-2xl shadow-2xl glass-dropdown sm:bottom-4 sm:right-4 sm:h-[min(520px,calc(100dvh-2rem))] sm:max-h-[calc(100dvh-2rem)] sm:w-[360px]"
        >
          ...
          <div className="presence-meshi-transcript flex-1 space-y-3 overflow-y-auto p-4" style={{ background: "var(--bg-primary)" }}>
          ...
          <div className="presence-meshi-composer flex items-center gap-2 border-t border-[var(--border-primary)] px-4 py-3" style={{ background: "var(--bg-secondary)" }}>
            <input
              ref={inputRef}
              type="text"
              value={input}
