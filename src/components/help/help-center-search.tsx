...
export function HelpCenterSearch({ articles }: HelpCenterSearchProps) {
  ...
  return (
    <div className="public-help-workspace grid gap-5 lg:grid-cols-[18rem_minmax(0,1fr)]">
      <aside className="help-filter-panel mesh-section h-fit p-4">
        <label htmlFor="help-search" className="text-sm font-semibold text-[var(--text-primary)]">
          Search help
        </label>
      ...
      <section className="min-w-0">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          ...
          {(query || activeCategory !== "all") && (
            <button type="button" onClick={resetSearch} className="mesh-choice inline-flex min-h-10 items-center justify-center rounded-xl px-3 text-sm font-semibold text-[var(--text-secondary)]">
              Reset
            </button>
          )}
        </div>
        ...
        {filteredArticles.length > 0 ? (
          <div className="grid gap-3">
            {filteredArticles.map((article) => {
              ...
              return (
                <article key={article.id} id={article.id} className="public-help-article public-panel p-5">
                  ...
                  <details className="mt-4 rounded-xl border border-[var(--border-primary)] bg-[var(--bg-primary)]/70 p-3">
                    <summary className="cursor-pointer text-sm font-semibold text-[var(--text-primary)]">How to handle it</summary>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {article.relatedLinks.map((link) => (
                      <Link key={`${article.id}-${link.href}`} href={link.href} className="inline-flex min-h-9 items-center gap-1.5 rounded-xl border border-[var(--border-primary)] px-3 text-xs font-semibold text-[var(--text-secondary)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]">
                        {link.label}
